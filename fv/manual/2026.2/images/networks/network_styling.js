  ...
  whenReady(function () {
    ...
    if (positionsFilename) {
      fetch(positionsFilename, { cache: "no-store" })
        ...
        .then(savedPositions => {
          window.savedNodePositions = savedPositions;
    ...
    function highlightHover(nodeId) {
      ...
      // only edges directly incident to the hovered node
    ...
    // Expose for inter-script use (e.g. clustered_network_styling.js)
    window.highlightHover = highlightHover;
    ...
    network.on("blurNode", () => {
      if (lockedNode) {
        highlightHover(lockedNode);
      } else if (lockedLegendGroup) {
        highlightGroup(lockedLegendGroup);
      } else {
        restoreAll();
      }
    });
    network.on("click", params => {
      if (params.nodes.length > 0) {
        if (config.clustered && network.isCluster(params.nodes[0])) return;
        lockedNode = params.nodes[0];
        lockedLegend = null;
    ...
    document.addEventListener("click", function(e) {
      ...
      if (networkCanvas && networkCanvas.contains(e.target)) return;
      if (legend.contains(e.target) || e.target === dropdown) return;
      lockedNode = null;
      lockedLegend = null;
      lockedLegendGroup = null;
      resetLegendStyle();
    ...
    dropdown.addEventListener("change", function() {
      ...
      if (config.clustered && typeof window.openClusterAndHighlight === "function") {
        window.openClusterAndHighlight(selectedNode);
        return;
      }

      network.selectNodes([selectedNode]);
      highlightHover(selectedNode);
    });
